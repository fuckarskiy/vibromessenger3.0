...
def profile():
    ...
    c=db()
    r=c.execute("SELECT username,avatar FROM users WHERE username=?",
...
def search():
    ...
    c=db()
    r=c.execute("SELECT username FROM users WHERE username LIKE ? LIMIT 20",
                (f"%{q}%",)).fetchall()
    c.close()
...
def history():
    if "user" not in session: return {"error":"auth"},401
    u=session["user"]; v=request.args["with"]
    c=db()
    r=c.execute("""
